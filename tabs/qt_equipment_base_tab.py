...
from core.ui_roles import CANDIDATE_ROLE
...
class OptionCombo(QWidget):
    ...
    def set_candidate_states(self, states):
        ...
        for index in range(self.combo.count()):
            ...
            # Keep the structured state in a dedicated role for delegates/tests;
            # the display label remains the human-readable marker text.
            self.combo.setItemData(index, state, CANDIDATE_ROLE)
...
class BaseEquipmentEditorTab(QWidget):
    ...
    def _rarity_name_fallback(self, mfg_id, part_id):
        """Resolve a blank rarity row through its NCS composition child.

        Some Repkit legendary compositions (notably Torgue 261:9) have no
        hand-written CSV description: the visible name lives on the linked
        primary augment (261:8, ``Outburst``).  Showing the raw ``Legendary``
        row made the picker look like it contained an unnamed legendary.
        """
    ...
    def on_mfg_change(self, *args):
        ...
        for _, r in rarity_rows.iterrows():
            ...
            if pd.isna(desc) or not str(desc).strip():
                desc = self._rarity_name_fallback(mfg_id, r['Part_ID'])
    ...
    def _roll_catalog_name(self, root_id, composition_ref, composition):
        ...
        if pd.isna(value) or str(value).strip().casefold() in {"", "nan", "none"}:
            value = self._rarity_name_fallback(root_id, part_id)
    ...
    def _setup_roll_menu(self):
        ...
        # The legacy QWidget button is already a tool button; keep its text
        # equal to the localized label so language refresh and accessibility
        # consumers do not see an unexplained emoji prefix.
        self.lucky_button.setText(texts["lucky"])
