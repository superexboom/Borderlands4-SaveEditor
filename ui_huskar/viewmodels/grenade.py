"""手雷编辑器 VM：对齐主线 QtGrenadeEditorTab 的差异配置。"""
...
from tabs.qt_grenade_editor_tab import load_grenade_data
...
@register("grenade", "GrenadePage.qml")
class GrenadeViewModel(EquipmentBaseViewModel):
    EQUIP_TYPE = "grenade"
    UI_LOC_KEY = "grenade_tab"
    STRINGS_SECTION = "grenade_tab"
    DEFAULT_SEED = 305
    ...
    def _declare_perk_groups(self):
        return [
            ...
            {"key": "mfg_perk", "mode": "picker", "title_key": "mfg_perks", "stackable": False},
    ...
    def _picker_categories(self, key):
        ...
        return [
            ...
            *[(k, labels.get(k, k)) for k in self._AUGMENT_FACETS],
    ...
    def _chip_options(self, key, mfg_id):
        if key == "element":
            ...
            if normal_ref:
                ...
                existing = {int(v) for v in element_rows["Part_ID"] if pd.notna(v)}
            ...
            return self._chip_option_list(key, element_rows, self._fmt_row)
        if key == "firmware":
            return self._chip_option_list(
                key, self._firmware_group_df("Grenade_perk_main_ID", self.SECONDARY_PARENT),
                self._fmt_row)
        return None
    ...
    def _build_skill_parts(self, mfg_id):
        ...
        for e in self._entries("mfg_perk"):
            for _ in range(int(e.get("count", 1))):
