"""护盾编辑器 VM：对齐主线 QtShieldEditorTab 的差异配置。"""
...
@register("shield", "ShieldPage.qml")
class ShieldViewModel(EquipmentBaseViewModel):
    EQUIP_TYPE = "shield"
    UI_LOC_KEY = "shield_tab"
    STRINGS_SECTION = "shield_tab"
    DEFAULT_SEED = 306
    ...
    def _declare_perk_groups(self):
        return [
            ...
            {"key": "primary", "mode": "picker", "title_key": "primary", "stackable": True},
            {"key": "secondary", "mode": "picker", "title_key": "secondary", "stackable": True},
        ]
    ...
    def _perk_items(self, main_id):
        items = []
        df = self.df_main[(self.df_main["Shield_perk_main_ID"] == main_id)
    ...
    def _on_mfg_changed_extra(self, mfg_id):
        ...
        if not self._imported_copy:
            # 新构建不允许跨厂商把不兼容的 augment 带过去
            for key in ("primary", "secondary"):
                entries = self._picker_sel.get(key, [])
                self._picker_sel[key] = [
                    e for e in entries if int(e["data"][0]) in allowed_parents]
    ...
    def _build_skill_parts(self, mfg_id):
        ...
        # primary/secondary 是独立的自然槽位，各自保留真实 parent id
        for key in ("primary", "secondary"):
