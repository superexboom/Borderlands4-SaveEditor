...
def main():
    """Launch the canonical HuskarUI application.

    This module remains importable because HuskarUI reuses its live workers
    and compatibility helpers, but it is no longer a second user-facing UI
    entry point. Running ``main_window.py`` must produce the same QML window
    as ``python -m ui_huskar``.
    """
    from ui_huskar.__main__ import main as huskar_main
